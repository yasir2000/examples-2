...
This example demonstrates how to use Serverless Framework with Go on AWS.
...
```bash
serverless invoke local --function streamer
```
...
View function logs:
```bash
serverless logs --function streamer
```

Tail logs in real-time:
```bash
serverless logs --function streamer --tail
```
