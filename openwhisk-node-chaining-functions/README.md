...
# OpenWhisk Serverless Chaining Functions example in NodeJS

This example demonstrates chaining functions in NodeJS.
...
```bash
serverless invoke local --function split
```
...
View function logs:
```bash
serverless logs --function split
```

Tail logs in real-time:
```bash
serverless logs --function split --tail
```
