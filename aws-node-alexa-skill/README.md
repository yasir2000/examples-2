...
# AWS Serverless Alexa Skill example in NodeJS
...
```bash
serverless invoke local --function luckyNumber
```
...
View function logs:
```bash
serverless logs --function luckyNumber
```

Tail logs in real-time:
```bash
serverless logs --function luckyNumber --tail
```
